...
```
Records:    e1(2)  e2(5)  e3(8)  ---watermark(6)---  e4(4)  e5(10)
                                            ↑
                                    e3 is on-time
                                    e4 is late (4 < watermark 6)
                                    e5 is on-time
```

Watermarks are generated by the Source. For Kafka the watermark is tracked **per
partition**: each partition's watermark is `max_timestamp_seen - max_out_of_orderness`
for that partition, and the watermark the job sees is the **minimum** across
partitions, so it waits for the slowest one. One shared watermark over all
partitions would follow the fastest partition, and records from slower partitions
(a parallel reader that runs ahead, catch-up after a restart) would be dropped as
late. It never moves backwards.

A partition that delivers no records for `KafkaPartitionIdleTimeout` (default 30s;
negative disables) stops holding the watermark back, so an empty or dead partition
cannot stall every window. A partition that has not delivered any record yet is
not considered. Source completion emits a final max watermark so pending windows
close.

Window operators can additionally be configured with allowed lateness; records
older than `current_watermark - allowed_lateness` are sent to a late-record side
output, or dropped if none is configured. **A dropped record produces no error or
log**, so watch the `weibo_window_late_records_total{disposition="dropped"}` counter:
a non-zero rate means the job is losing data to lateness (raise the out-of-orderness
bound, or add a late-record sink to capture them).
